...
function f_tcalGetHTML(d_date) {
  ...
  d_firstDay.setDate(
    1 - ((7 + d_firstDay.getDay() - A_TCALCONF.weekstart) % 7)
  );
  ...
  for (var i = 0; i < 7; i++)
    s_html +=
      "<th>" + A_TCALCONF.weekdays[(A_TCALCONF.weekstart + i) % 7] + "</th>";
  s_html += "</tr>";
...
function f_tcalRelDate(d_date, d_diff, s_units) {
  ...
  return (
    ' onclick="f_tcalUpdate(' + d_result.valueOf() + (d_diff ? ",1" : "") + ')"'
  );
}
...
function f_tcalParseDate(s_date, s_format) {
  ...
  var s_val,
    d_date = f_tcalResetTime(new Date());
  d_date.setDate(1);
...
function f_tcalGenerateDate(d_date, s_format) {
  ...
  for (var n = 0; n < s_format.length; n++) {
    s_char = s_format.charAt(n);
    s_date += A_TCALTOKENS_IDX[s_char]
      ? A_TCALTOKENS_IDX[s_char]["g"](d_date)
      : s_char;
  }
  return s_date;
}
